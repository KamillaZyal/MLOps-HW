...
def get_datasets(
    path, n1, n2, is_pred=False, mnist_dataset=True, has_labels=True, show_image=False
):
    get_data_dvc(path)
    transform = transforms.Compose(
        [transforms.ToTensor(), transforms.Normalize((n1,), (n2,))]
    )
    if mnist_dataset:
        return MNISTDataset(path, transform=transform, is_pred=is_pred)
    else:
        return get_testdata(
            path, transform=transform, has_labels=has_labels, show_image=show_image
        )
...
class MNISTDataModule(pl.LightningDataModule):
    def __init__(
        self, batch_size, num_workers, val_size, path_train_val, path_test, n1, n2
    ):
        super().__init__()

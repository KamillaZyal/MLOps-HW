...
def main(cfg: DictConfig) -> None:
    ...
    with mlflow.start_run():
        ...
        for inp in data_test:
            inp = torch.reshape(
                inp, (cfg.model.in_channels, cfg.model.in_channels, 28, 28)
            )
            predictions_list.append(
                model_mlflow.predict(inp.detach().cpu().numpy())["predictions"].argmax()
            )
        ...
        print(metrics)
        df_pred.to_csv(
            cfg.data.pred_onnx_data_path, columns=["predicted_label"], index=False
        )
        print(f"Predictions saved to {cfg.data.pred_onnx_data_path}")

        # new examples
        labels, data_test = get_datasets(
            cfg.data.exp_data_path,
            n1=cfg.preprocessing.n1,
            n2=cfg.preprocessing.n2,
            mnist_dataset=False,
            has_labels=True,
        ...
        for i, inp in enumerate(data_test):
            ...
            prediction = model_mlflow.predict(inp.detach().cpu().numpy())[
                "predictions"
            ].argmax()
